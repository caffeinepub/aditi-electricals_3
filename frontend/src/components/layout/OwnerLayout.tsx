...
const navItems: { page: OwnerPage; label: string; icon: React.ReactNode }[] = [
  { page: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={18} /> },
  { page: 'workers', label: 'Workers', icon: <Users size={18} /> },
  ...
  { page: 'holidays', label: 'Holidays', icon: <Calendar size={18} /> },
];

export default function OwnerLayout({ currentPage, onNavigate, onLogout, children }: OwnerLayoutProps) {
  const { user } = useAuth();
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const handleNavigate = (page: OwnerPage) => {
    onNavigate(page);
