...
export default function TodayAttendanceMap({ attendanceData }: TodayAttendanceMapProps) {
  const [selectedMarker, setSelectedMarker] = useState<AttendanceWithWorker | null>(null);
  ...
  const center = React.useMemo(() => {
    ...
    const avgLat = validRecords.reduce((sum, d) => sum + (d.record.latitude ?? 0), 0) / validRecords.length;
    const avgLng = validRecords.reduce((sum, d) => sum + (d.record.longitude ?? 0), 0) / validRecords.length;
    return { lat: avgLat, lng: avgLng };
  }, [validRecords]);

  const zoom = validRecords.length > 0 ? 14 : 5;
  ...
  if (validRecords.length === 0) {
    return (
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: '14px' }}>No GPS data available for today's attendance.</p>
      ...
      <div style={{ marginTop: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {validRecords.map(({ record, worker }) => (
            <div
              key={record.recordId}
              onClick={() => setSelectedMarker(selectedMarker?.record.recordId === record.recordId ? null : { record, worker })}
              style={{
                backgroundColor: selectedMarker?.record.recordId === record.recordId ? '#EFF6FF' : '#fff',
                  ...
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                    {record.photo && (
                      <img
                        ...
                        alt={`${worker.name} attendance photo`}
                        style={{
                          width: '80px',
                          height: '80px',
                          objectFit: 'cover',
                    ...
                    <div>
                      ...
                      <p style={{ margin: '0 0 4px 0', fontSize: '13px', color: '#444' }}>
                        <strong>Longitude:</strong> {record.longitude?.toFixed(6)}
                      </p>
                      {!record.photo && (
                        <p style={{ margin: '0', fontSize: '12px', color: '#aaa' }}>No photo available</p>
                      )}
                    </div>
                  </div>
                </div>
