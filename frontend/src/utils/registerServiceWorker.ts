      ...
      try {
        navigator.serviceWorker.ready.then(reg => {
          ...
          if ('vibrate' in navigator) {
            (navigator as Navigator & { vibrate: (pattern: number[]) => void }).vibrate([200, 100, 200]);
          }
        });
      } catch {
...
export function scheduleDailyNotifications(): void {
  ...
  const schedule = [
    { hour: 9, minute: 0, title: 'Aditi Electricals', body: '⏰ Time to mark your attendance!' },
    { hour: 9, minute: 15, title: 'Aditi Electricals', body: '🔔 Start work reminder - Have you marked attendance?' },
    { hour: 14, minute: 0, title: 'Aditi Electricals', body: '🕑 2PM Check-in - Please confirm you resumed work!' },
  ];
