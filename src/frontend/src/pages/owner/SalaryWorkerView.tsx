        ...
        <Card className="card-shadow border-0 overflow-hidden">
          <CardHeader className="bg-[var(--header-bg)] text-white pb-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {workerName && (
                  <p className="text-white/70 text-sm mt-0.5">{workerName}</p>
                )}
              </div>
          ...
          <CardContent className="p-0 divide-y divide-border">
            {[
              ...
              {
                label: "Per Day",
                value: `₹${calc.perDay.toLocaleString("en-IN")}`,
              },
              {
                label: "Present Days",
                value: String(Number(record.presentDays)),
              },
              {
                label: "Absent Days",
                value: String(Number(record.absentDays)),
              },
              { label: "Cut Days", value: String(Number(record.cutDays)) },
              {
                label: "Company Holidays",
                value: String(Number(record.companyHolidays)),
              },
              {
                label: "Final Deductible",
                value: String(calc.finalDed),
                highlight: true,
              },
              {
                label: "Cut Amount",
                value: `₹${Math.abs(calc.cutAmt).toLocaleString("en-IN")}`,
                highlight: true,
              },
              {
                label: "Advance",
                value: `₹${calc.adv.toLocaleString("en-IN")}`,
              },
              {
                label: "Carry Forward",
              ...
              <div
                key={row.label}
                className={`flex justify-between px-4 py-2.5 text-sm ${row.highlight ? "bg-orange-50" : ""}`}
              >
                <span
                  className={
                    row.highlight
                ...
                <span
                  className={`font-semibold ${row.highlight ? "text-orange-700" : "text-foreground"}`}
                >
                  {row.value}
                </span>
              </div>
